...
**Local/OpenRouter requests fail with HTTP 402 ("This request requires more credits, or fewer max_tokens")**: The request is using the model's advertised max output tokens as the request default. Advertised capability (for example, GPT-6 Astra 128K) is a provider ceiling, not a default. Keep provider capability, configured request default, and explicit user override separate; the local model-selection boundary must preserve configured `max_output_tokens` instead of substituting the runtime catalog's advertised `maxOutputTokens`.
