...
import { getCronFilePath, readCronFile } from "@/cron/cron-file";
...
describe("listener cron command validation", () => {
  ...
  test("clears a legacy invalid failure when the cron is corrected", async () => {
    ...
    const persisted = readCronFile();
    const persistedTask = persisted.tasks.find(
      (candidate) => candidate.id === task.id,
    );
    if (!persistedTask) throw new Error("expected persisted cron task");
    persistedTask.cron = "0-60 * * * *";
    writeFileSync(getCronFilePath(), JSON.stringify(persisted, null, 2));
