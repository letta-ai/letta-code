...
import type { LocalProjectSettings, ProjectSettings, Settings } from "../settings-manager";
...
describe("Settings Manager - Project Settings", () => {
  ...
  test("Load project settings creates defaults if none exist", async () => {
    const projectSettings = await settingsManager.loadProjectSettings(testProjectDir);
...
describe("Settings Manager - Local Project Settings", () => {
  ...
  test("Load local project settings creates defaults if none exist", async () => {
    const localSettings = await settingsManager.loadLocalProjectSettings(testProjectDir);
  ...
  test("Local project settings persist to disk", async () => {
    ...
    settingsManager.reset();
    await settingsManager.initialize();
    const reloaded = await settingsManager.loadLocalProjectSettings(testProjectDir);
  ...
  test("Throw error if accessing local project settings before loading", async () => {
    expect(() => settingsManager.getLocalProjectSettings(testProjectDir)).toThrow(
      "Local project settings for"
    );
  });
});
