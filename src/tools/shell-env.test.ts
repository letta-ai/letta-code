...
test("getShellEnv injects Pierre MemFS git header for Bash git commands", () => {
  ...
  expect(env.GIT_CONFIG_VALUE_0).toBe("x-letta-memfs-backend: pierre");
});
...
test("getShellEnv injects Pierre MemFS git header for Desktop proxy git commands", () => {
  ...
  expect(env.GIT_CONFIG_VALUE_1).toBe("x-letta-memfs-backend: pierre");
  expect(env.GIT_CONFIG_KEY_2).toBe(
    "http.http://localhost:57294/v1/git/.extraHeader",
  );
  expect(env.GIT_CONFIG_VALUE_2).toBe("x-letta-memfs-backend: pierre");
});
