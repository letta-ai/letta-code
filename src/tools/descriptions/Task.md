...
- Agents always run in the background. The tool result will include a task ID and an output_file path, and except for silent memory tasks, you will be notified automatically via a <task-notification> message when it completes — no need to poll. If you need interim progress before then, use the TaskOutput tool with the task ID. You can continue working while agents run.
