...
Subagents run as isolated stateless sessions with read-only tools by default (Read, Grep, Glob) and no access to your memory or conversation — put ALL context they need in the prompt. They are told their final text IS the return value, so they return raw data. Concurrency is capped (excess agent() calls queue); a lifetime cap of 1000 agents per run is the runaway-loop backstop.

Every run persists its script and a journal of each subagent's outcome under ~/.letta/workflows/runs/<runId>/ (the run ID is in the tool result). To resume after a failure or script edit, re-invoke with resumeFromRunId — unchanged agent() calls replay instantly. Before diagnosing an empty or unexpected result, read that run's journal.jsonl; it records each agent's actual return value.
