...
export async function executeWorkflow(
  ...
  options: Omit<RunWorkflowOptions, "signal">,
  ...
  // Completion timing may change on replay, but an identical call must stay
  // with its pipeline item. Helper invocations and items each get a path.
  const branches = new AsyncLocalStorage<{
    path: (string | number)[];
    next: number;
  }>();
  const rootBranch = { path: [] as (string | number)[], next: 0 };
  const branch = () => branches.getStore() ?? rootBranch;
  ...
  function agent(prompt: unknown, callOptions?: unknown): Promise<unknown> {
    ...
    // Scripts can forget to await a call; cleanup must not turn its rejection
    // into an unhandled error. Awaiting it still observes the original error.
    void pending.catch(() => {});
    return pending;
  }
  ...
  async function callAgent(
    ...
    // Older journals omitted branch identity (and, before that, execution
    // defaults). Rerun those calls rather than assigning results to wrong items.
  ...
  async function parallel(thunks: unknown): Promise<unknown[]> {
    ...
    const parent = branch();
    const group = parent.next++;
    return Promise.all(
      thunks.map((thunk, index) =>
        branches.run(
          { path: [...parent.path, "parallel", group, index], next: 0 },
          async () => {
  ...
  async function pipeline(
    ...
    return Promise.all(
      items.map(async (item, index) => {
        ...
        for (const [stageIndex, stage] of stageFns.entries()) {
          try {
            value = await branches.run(
              {
                path: [...parent.path, "pipeline", group, index, stageIndex],
                next: 0,
              },
  ...
  function executeScript(
    ...
    async function workflowHook(
      ...
      try {
        childScript = readFileSync(
          resolve(options.workingDirectory ?? process.cwd(), scriptPath),
          "utf8",
        );
      } catch (error) {
        throw new Error(
      ...
      const parent = branch();
      return branches.run(
        { path: [...parent.path, "workflow", parent.next++], next: 0 },
        () => executeScript(childScript, childArgs, depth + 1, childMeta.name),
      );
    }
