...
The primary agent's context (its prompts, skills, and external memory files) is stored in a local backend memory filesystem rooted at `$MEMORY_DIR` (usually under `~/.letta/lc-local-backend/memfs/`). Changes to these files are reflected in the primary agent's context after they are committed to the local MemFS git repo. No remote push is required unless the user explicitly configured an optional mirror.
...
2. a `multi_transcript_reflection_payload` manifest. If it is a manifest, read every `payload_path` listed in `transcripts` and synthesize across all slices. Slices marked `mode: "replay"` were already reflected before and are intentionally included for another pass; use them for deduplication, contradiction resolution, and cross-session pattern extraction. If the manifest includes `user_instruction`, use it as the requested reflection focus while still only persisting durable memory-worthy learnings, or
3. an `auto_transcript_reflection_candidates` manifest. If it is a transcript candidates manifest, you are in selector mode: do **not** update memory and do **not** commit. Choose up to `max_selected` conversations likely to contain durable memory updates and return strict JSON as your final response with shape `{ "selected_conversations": [{ "conversation_id": "...", "reason": "...", "priority": "high" }] }`. If the manifest includes `user_instruction`, prefer candidates that help satisfy it. Treat candidate summaries/descriptions as weak metadata, not facts. Prefer explicit user corrections, repeated preferences, coding/review/commit conventions, repo or workflow gotchas, contradictions with current memory, and repeated agent failures. Avoid one-off debugging, transient task status, duplicated/redundant candidates, and fully reflected conversations unless useful for deduplication or contradiction resolution. If nothing looks memory-worthy, return `{ "selected_conversations": [] }`. Do not write a selection file; the parent harness parses your final response.
...
4. **Commit reference** — Local commit hash (or "no commit" if nothing was persisted)
...
4. **Always commit durable changes** — Your work is wasted if it isn't committed locally
