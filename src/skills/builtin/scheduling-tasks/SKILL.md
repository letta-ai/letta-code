...
- **`--computer <deviceId>`** — the work needs a specific connected computer (its filesystem, services, or credentials). Get the deviceId from `letta computers list`. If that computer is offline at fire time, the run fails instead of switching filesystems and credentials in the cloud sandbox.
