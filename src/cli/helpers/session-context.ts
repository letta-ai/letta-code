...
- Do NOT redirect native stderr (\`2>&1\`, \`2>$null\`) in Bash tool commands: on Windows PowerShell 5.1 that turns benign stderr (e.g. git's LF/CRLF warnings) into exit code 1. The Bash tool already captures stderr separately; for git, set \`$env:GIT_REDIRECT_STDERR="2>&1"\` instead
`;
}
