...
export async function resolveDiscordInboundAttachments(params: {
  ...
  for (const attachment of params.rawAttachments) {
    ...
    try {
      ...
      // Auto-transcribe Discord audio attachments when enabled and an API key
      // is available. Discord voice messages arrive as ordinary audio
      // attachments, so unlike Telegram there is no separate voice flag.
