...
The gateway submits caller-supplied input through the same queue and turn
lifecycle as channel messages. Preserve the original message payload (roles,
attachments, IDs) instead of rewriting it into one user message, and return the
actual acceptance disposition. Remote hosts (for example Cloud) depend on this
to run app-server message batches through channel progress and failure
handling.

Keep plan/TODO progress generic: emit structured plan updates (the agent's TODO
items and statuses) as progress events after `TodoWrite`/`UpdatePlan` succeeds
and let the host render them. Do not put Slack-specific rendering in the
harness.
