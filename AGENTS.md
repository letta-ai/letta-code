...
- **Awaiting `AbortSignal.timeout()`'s abort event can hang the Windows test runner** once only unref'd handles remain; the pinned Bun predates the upstream fix. Drive deadlines and cleanup off `setTimeout` plus an independent `Promise.race` instead of awaiting the signal's abort event.
